...
class lt {
  ...
  acquireAudioElement() {
    var s, r;
    const e = document.getElementById("om-persistent-player"), t = document.getElementById(C);
    if ((s = this.audio) != null && s.isConnected)
      return this.audio;
    if ((r = this.audio) != null && r.src && !this.audio.isConnected)
      return (e ?? document.body).appendChild(this.audio), this.audio;
    if (t != null && t.isConnected)
      return this.wireAudioElement(t), this.audio = t, t;
    if (t != null && t.src && !t.isConnected)
      return (e ?? document.body).appendChild(t), this.wireAudioElement(t), this.audio = t, t;
    let i = t;
    return i || (i = document.createElement("audio"), i.id = C, i.setAttribute("data-turbo-permanent", ""), i.preload = "auto", i.setAttribute("playsinline", ""), i.hidden = !0), i.isConnected || (e ?? document.body).appendChild(i), this.wireAudioElement(i), this.audio = i, i;
  }
  wireAudioElement(e) {
  ...
  ensureAudioConnected() {
    var i, s;
    const e = document.getElementById("om-persistent-player"), t = document.getElementById(C);
    if (!((i = this.audio) != null && i.isConnected)) {
      if ((s = this.audio) != null && s.src && !this.audio.isConnected) {
        (e ?? document.body).appendChild(this.audio);
        return;
      }
      if (t != null && t.src && !t.isConnected && (e ?? document.body).appendChild(t), t != null && t.isConnected) {
        this.audio !== t && (this.wireAudioElement(t), this.audio = t);
        return;
      }
  ...
  peekAudioElement() {
    ...
    return e != null && e.isConnected ? (this.audio !== e && (this.wireAudioElement(e), this.audio = e), e) : (t = this.audio) != null && t.isConnected ? this.audio : this.audio ?? e;
  }
  ...
  hasActivePlayback() {
    const e = this.audio ?? document.getElementById(C);
...
class dt {
  constructor() {
    ...
    e != null && e.volume && (this.volume = e.volume), e != null && e.repeat && (this.repeat = e.repeat), e != null && e.shuffle && (this.shuffle = e.shuffle), e != null && e.trackSlug && this.engine.primeMediaSession(
      ...
      this.isNavigatingPlayback() && this.guardContinuousPlayback();
    ...
    }, !0), window.addEventListener("pagehide", () => this.persist(!0)), window.addEventListener("beforeunload", () => this.persist(!0)), window.addEventListener("turbo:before-visit", () => {
      ...
      this.wireAudioNavigationGuard(), this.isNavigatingPlayback() && (this.guardContinuousPlayback(), requestAnimationFrame(() => {
        this.isNavigatingPlayback() && this.guardContinuousPlayback();
      }), window.setTimeout(() => {
        this.isNavigatingPlayback() && this.guardContinuousPlayback(), this.finalizeNavigationPlayback();
      }, 320));
    });
  }
  ...
  clearNavigationLock() {
    this.wasPlayingBeforeNav = !1, this.navigationPlaybackLock = !1, this.navResumeGen += 1;
  }
  captureNavigationIntent() {
    const e = this.engine.peekAudioElement();
    !(e != null && e.src) || e.ended || (this.isPlaying() || this.intendedPlaying && e.currentTime > 0.25) && (this.wasPlayingBeforeNav = !0, this.navigationPlaybackLock = !0);
  }
  isLivePlayback() {
    const e = this.engine.getAudioElement();
  ...
  async ensureRestored() {
    ...
    const e = this.engine.getAudioElement();
    if (!(this.engine.getCurrentTrack() && (e != null && e.src))) {
      if (this.restoreFn) {
  ...
  finalizeNavigationPlayback() {
    const e = this.engine.peekAudioElement();
    e != null && e.src && !e.paused && !e.ended && this.markPlaybackIntent(), this.clearNavigationLock(), this.notify();
  }
  ...
  guardContinuousPlayback() {
    if (!this.isNavigatingPlayback()) return;
    let e = this.engine.peekAudioElement();
    if (e != null && e.src && !(!e.isConnected && (this.engine.ensureAudioConnected(), e = this.engine.peekAudioElement(), !(e != null && e.src)))) {
      if (!e.paused && !e.ended) {
        this.markPlaybackIntent();
        return;
      }
      this.engine.resumeIfPaused(), this.armNavigationResume();
    }
  }
  armNavigationResume() {
    const e = ++this.navResumeGen, t = [0, 40, 120, 280], i = () => {
      if (e !== this.navResumeGen || !this.isNavigatingPlayback()) return;
      let s = this.engine.peekAudioElement();
      if (!(!(s != null && s.src) || s.ended) && !(!s.isConnected && (this.engine.ensureAudioConnected(), s = this.engine.peekAudioElement(), !(s != null && s.src)))) {
        if (!s.paused) {
          this.markPlaybackIntent();
          return;
        }
        s.play().then(() => {
          s.paused || (this.markPlaybackIntent(), this.engine.ensurePlaybackRunning());
        }).catch(() => {
        });
      }
    };
    t.forEach((s) => {
      s === 0 ? i() : window.setTimeout(i, s);
    });
  }
  subscribe(e) {
    return this.listeners.add(e), () => this.listeners.delete(e);
  ...
  writeSaved(e = !1) {
    const t = this.engine.getCurrentTrack(), i = this.getPositionMs(), s = this.restoringPositionMs !== null && i < 1e3 ? this.restoringPositionMs : i, r = {
...
class K extends I {
  ...
  connectedCallback() {
    if (super.connectedCallback(), this.client = new Ye(this.apiBase), this.unsub = this.store.subscribe(() => {
      ...
      if (t != null && t.src && !t.ended)
        this.visible = !0;
      else {
        const i = this.store.loadSaved();
        if (!this.store.getCurrentTrack() && !this.store.engine.hasActivePlayback() && !!(i != null && i.trackSlug)) {
          const r = () => {
            this.isConnected && this.restoreSessionPublic();
          };
          typeof requestIdleCallback == "function" ? requestIdleCallback(r, { timeout: 2e3 }) : setTimeout(r, 120);
        }
      }
    }
    this.store.getCurrentTrack() && (this.visible = !0), this.addEventListener("om:cmd-next", () => this.store.next()), this.addEventListener("om:cmd-prev", () => this.store.prev()), this.mode === "full" ? this.hydrateQueueFromPage() : this.mode === "embed" && this.track && (this.autoPlay ? this.playSlug(this.track) : this.scheduleEmbedPreload()), (this.mode === "mini" || this.mode === "full" || this.mode === "embed") && this.startProgressTimer(), this.heartClickHandler = (t) => {
  ...
  disconnectedCallback() {
    var t, i;
    this.stopProgressTimer(), this.heartClickHandler && (this.renderRoot.removeEventListener("click", this.heartClickHandler), this.heartClickHandler = null), (t = this.unsub) == null || t.call(this), (i = this.unsubFav) == null || i.call(this), this.unsub = null, this.unsubFav = null, super.disconnectedCallback();
  }
  requestUpdate(t, i) {
  ...
  async playQueueIndexAsync(t) {
    if (this.client)
      try {
        const i = await this.resolveQueueStreams(this.queue);
        this.store.originalQueue = this.mergeQueueStreams(i, this.store.originalQueue), this.store.queue = [...i], this.store.setQueue(i, t);
      } catch {
  ...
  renderCover(t = "sm") {
    ...
    return i ? p`<img class=${s} src=${i} alt=${this.displayTitle} loading="lazy">` : p`<div class=${s}></div>`;
  }
  ...
  async loadQueue(t) {
    ...
    if (this.hydrateQueueFromPage()) {
      if (t && this.store.queue.length) {
        const n = this.album || ((s = this.store.originalQueue[0]) == null ? void 0 : s.albumSlug) || "";
        if (n && this.shouldSkipAlbumStart(n)) {
          this.visible = !0;
          return;
        }
        this.loading = !0, this.error = "";
        try {
          const l = await this.resolveQueueStreams(this.store.originalQueue);
          this.store.originalQueue = l, this.store.queue = [...l], this.store.setQueue(l, 0, 0, !0, !0), this.visible = !0;
        } catch {
    ...
    const i = (r = this.store.originalQueue[0]) == null ? void 0 : r.albumSlug;
    if (this.album && i === this.album && this.store.originalQueue.length > 0) {
      if (this.store.queue = [...this.store.originalQueue], t && this.store.queue.length) {
        if (this.shouldSkipAlbumStart(this.album)) {
          this.visible = !0;
          return;
        }
        this.store.setQueue(this.store.originalQueue, 0, 0, !0, !0), this.visible = !0;
      }
      return;
    }
    this.loading = !0, this.error = "";
    try {
      let n = [];
      if (this.album ? n = (await this.client.getAlbumTracks(this.album)).data : this.playlist && (n = (await this.client.getPlaylistTracks(this.playlist)).tracks), this.store.originalQueue = n, this.store.queue = [...n], t && n.length) {
        if (this.album && this.shouldSkipAlbumStart(this.album)) {
          this.visible = !0;
          return;
        }
        this.store.setQueue(n, 0, 0, !0, !0), this.visible = !0;
      }
    } catch {
      this.error = "Не удалось загрузить треки";
    } finally {
  ...
  shouldSkipAlbumStart(t) {
    if (!this.store.isPlaying()) return !1;
    const i = this.store.getCurrentTrack();
    return (i == null ? void 0 : i.albumSlug) === t, !0;
  }
  async playAlbum(t) {
    if (this.client && !this.shouldSkipAlbumStart(t)) {
      this.loading = !0;
      try {
    ...
    if (this.mode === "full") {
      ...
      return p`
        <div class="player player--full" role="region" aria-label="Плеер альбома">
          ...
          ${this.error ? p`<div class="state state--error">${this.error}</div>` : m}
  ...
  async restoreSessionPublic() {
    ...
    if (this.store.engine.hasActivePlayback() || this.store.engine.getCurrentTrack() && (t != null && t.src)) {
      this.visible = !0;
      return;
    }
    const i = this.store.loadSaved();
    if (!(i != null && i.trackSlug)) return;
    this.visible = !0, this.loading = !0;
    const s = this.store.shouldAutoplayRestore(i.wasPlaying === !0);
    this.restoreInFlight = (async () => {
      var r;
      try {
        i.albumSlug && ((r = i.queueSlugs) == null ? void 0 : r.length) > 1 ? await this.restoreQueuePublic(
          i.albumSlug,
          i.trackSlug,
          i.queueSlugs,
          i.queueIndex,
          i.positionMs,
          s
        ) : await this.loadTrackPublic(i.trackSlug, i.positionMs, s);
      } finally {
        this.isConnected && (this.loading = !1);
      }
    })();
  ...
  async mediaPlayPausePublic() {
    this.trySyncPlayPause() || (await this.restoreSessionPublic(), this.store.getCurrentTrack() && (await this.store.waitUntilReady(), this.visible = !0, this.store.toggleFromUserGesture()));
  }
